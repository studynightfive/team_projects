...
- 2026-07-16 用户工作区首页升级为企业 AI 搜索工作台，复用 M01 V2 壳层与 tokens，并新增搜索结果、深度研究、知识空间、数据源、历史、收藏和搜索设置路由。2026-07-17 项目负责人确认搜索设置不显示在用户主导航，`/settings` 页面与底部账号菜单入口继续保留；用户主导航将“AI 助手”放在第二位、紧随“AI 搜索”，移动端底部四个高频入口顺序不变。当前业务数据与交互均使用严格类型的本地 Mock；Markdown 通过 `markdown-it@14.1.0` 禁用 HTML 后再由 `dompurify@3.3.0` 二次过滤。真实搜索、文件上传、收藏持久化与权限联调继续以 OpenAPI 契约为准。
