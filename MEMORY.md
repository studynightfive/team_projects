...
- 内建权限和角色在启动时始终幂等初始化；演示账号只在非生产环境、`AUTO_SEED_DEMO_DATA=true` 且显式提供合规演示口令时播种。生产首管理员通过 `backend/scripts/bootstrap_admin.py` 交互创建，不保留默认凭据；该受信任脚本直接执行唯一首管理员初始化，不能复用禁止分配超级管理员角色的普通用户管理服务。容器内执行时使用绝对路径 `/app/backend/scripts/bootstrap_admin.py`。
...
- 答案缓存先查标准化问题的精确键，再使用本次检索复用的查询向量做语义相似匹配；默认阈值为 `0.92`，且问题类型、
...
- 首轮回答继续使用精确与高置信度语义缓存；同一对话中的完整独立问题也允许读写缓存，只有“那它呢”
...
- 平台运营看板以商品实体而非完整问题文本统计热度；当前没有独立商品主数据表，因此使用 RAG 最终
  第一引用的文档 ID 作为商品标识，文档标题作为名称快照。同一商品的不同问法归并，不同文档不得
  因问题相似而合并。
- `retrieval_metrics` 只保存主商品标识和名称等结构化事实，不保存用户问题或答案正文；文档删除后
  历史运营数据仍需保留，因此商品标识不建立文档外键。无引用回答不进入热门商品统计。
- 商品资料推荐采用“一件商品一份文档”，正文同时维护商品详情和店铺位置信息。一份文档包含多个
  商品时，现有指标无法稳定拆分；更细粒度分析必须引入商品、店铺及文档映射表，不能依赖关键词猜测。

## 2026-07-31 查询预处理、观测与部门口径

- 用户查询在 RAG 前统一执行一次语义预处理：关键词只能作为风险候选，聊天模型同时完成合法意图判定与
  检索改写。输入框预检和最终提交复用 Redis 短期结果，模型不可用时按 `RAG_SEMANTIC_GUARD_FAIL_CLOSED`
  决定降级策略，禁止恢复基于关键词直接拒绝的误伤行为。
- 高相似答案缓存必须复用真实查询 Embedding，并同时校验问题类型、否定、数字和英文实体；缓存作用域继续
  包含用户、知识库集合、文档版本、模型和检索参数，任何知识或模型版本变化都必须自然失效。
- Langfuse 是可选观测而不是业务依赖；客户端进程内单例，默认不采集问题、片段和答案正文，初始化、上报
  或关闭失败均不得阻断 RAG。根问答、预处理、检索、重排和生成应位于同一 trace。
- 运营看板中文档和知识贡献按知识库所属部门归属，不按上传者当前部门归属；超级管理员可切换部门，其他
  管理员固定本部门，前端必须核对响应 `scope.department_id`，防止迟到响应覆盖当前选择。
- 文档删除后的 UI 采用本地立即移除加单次静默校准，不切换整页加载态；重新处理请求必须携带切分方法、
  大小和重叠字符，并完整重建 Markdown、分块和向量索引。

## 2026-08-02 检索指标部门归属

- `retrieval_metrics` 的部门归属必须来自实际命中的知识库；未命中时使用请求中用户明确选择的首个知识库，
  不能使用提问用户的 `department_id`。多知识库请求使用 `kb_ids` 时也必须写入有效知识库 ID。
- 看板按部门统计检索指标时，以关联知识库当前所属部门为事实来源；只有缺少知识库 ID 的历史指标才允许回退
  到指标中的部门快照。迁移 `0024` 先按主商品文档恢复知识库，再按同一用户会话中紧邻指标的助手消息恢复
  旧流式问答归属，并同步知识库部门快照。
- 运行环境由部署配置决定；没有真实后端切换语义的“演示/生产”前端选择器不得展示，避免用户误以为它会
  切换数据源。
