...
## 2026-07-24 RAG 流式回答与相似问题缓存

- AI 搜索的无会话问答统一使用 `POST /api/v1/retrieval/answer/stream`，事件顺序为
  `start`、`stage`、`citation`、`delta`、`done` 或 `error`；前端使用原生 Fetch、
  ReadableStream、TextDecoder 和 AbortController 消费，并允许用户主动停止生成。
- “回答处理过程”只展示缓存检查、检索、重排、生成等可审计阶段及结果摘要，不展示或保存
  模型私有思维链；供应商返回的 `<think>` 内容在跨流式分片场景下也必须过滤。
- 相似问题缓存按用户、可访问知识库、文档版本指纹、聊天/Embedding/Rerank 模型和检索参数
  共同隔离；仅在高阈值本地相似度判定后复用已完整生成的答案，Redis 故障时降级为正常 RAG，
  取消或失败的生成不得写入缓存。

## 2026-07-24 文档原文、Markdown、分块与 OCR 展示

- 文档详情默认展示上传原件，并通过 `GET /api/v1/documents/{id}/original` 鉴权读取；
  原件不得暴露为静态文件地址或返回宿主机路径，Office 格式无法可靠内联时提供鉴权下载。
- Markdown 仍是后台清洗、切分和索引的标准中间格式，服务端清洗后由前端 `SafeMarkdown`
  再次过滤；原文、Markdown 和活动分块必须作为三个独立视图展示，不能互相冒充。
- OCR 摘要保存在现有 `manifest.json`，包含状态、语言、平均置信度和人工复核标记，不为此
  增加数据库列；历史文档缺少摘要时由服务层提供稳定兼容状态。
- 分块接口统一返回分页数据，并展示标题、页码、字符范围、Token 估算、索引代次和向量状态；
  前端按页读取，禁止一次加载大型文档的全部分块。

## 2026-07-24 业务看板与用户激励

- 检索业务 KPI 使用独立的 `retrieval_metrics` 结构化事实表，不从审计日志自由文本反向解析；
  指标只保存用户、部门、知识库、命中数、生成/缓存状态和耗时，不保存问题或答案原文。
- 搜索、非流式回答和流式回答完成时写入指标，`event_type + request_id` 唯一约束负责请求回放去重；
  指标写入失败不得阻断 RAG 主流程。
- 管理看板按 7/30/90 天和服务端部门边界聚合知识覆盖、活跃检索、有效回答、未命中、文档处理成功率、
  答案缓存命中率和平均响应时间；超级管理员可跨部门，部门管理员只能查看本部门。
- 用户贡献积分从数据库事实动态计算：同一用户的 `ready + is_active_index` 文档按 `content_hash`
  去重，每份 10 分；搜索、失败文档和重复处理不计分。徽章和部门榜单由积分实时推导，同分用户共享名次。
- 普通用户只通过 `/api/v1/me/incentives` 查看自己的积分、徽章、部门排名和分页贡献明细；
  `/api/v1/admin/dashboard` 返回分页部门榜单。Windows 缺少 IANA 时区数据时，仅对系统默认
  `Asia/Shanghai` 使用标准库 UTC+8 回退，Linux 和云端继续使用 `ZoneInfo`。

## 2026-07-24 移动端关键流程

- 375px 下页面级横向滚动始终视为缺陷；宽表只允许在 `.data-table-scroll` 内滚动，含主要操作的表格使用
  `mobile-sticky-actions` 固定最后一列，保证手机端无需滑到表尾即可操作。
- Ant Design Drawer 在 767px 以下统一使用全视口宽度；Drawer、Modal、自定义搜索上下文、原文预览和筛选底部面板
  必须兼容 `100dvh` 与安全区，关闭、取消、确认和表单控件的触控高度不低于 44px。
- 移动端浏览器门禁不能只检查静态路由；至少打开用户编辑 Drawer、答案导出 Modal 和原始结果筛选面板，并同时验证
  浮层边界、操作高度、根节点溢出、控制台错误和意外业务 API 请求。真实个人知识库上传与文档三视图由组件测试补充。

## 2026-07-24 文档批量处理与回收站

- 文档删除采用 30 天软删除：写入 `deleted_at`、`deleted_by` 和 `purge_after` 后立即停用分块并从 RAG 投影移除，
  列表、统计、指纹和积分均忽略已删除文档；Worker 定时物理清理到期记录与存储文件。
- 回收站恢复不会直接复用旧向量。恢复时清除删除标记并创建真实 `DocumentTask`，重新生成 Markdown、分块和向量，
  避免 Embedding 模型切换后混用历史索引。
- 批量重处理和恢复复用逐文档任务，接口返回任务 ID 列表；前端轮询现有任务接口聚合进度，不新增虚假的批次状态表。
- 上传文件先进入浏览器内存队列，允许重复追加、移除和统一确认；确认后继续复用现有多文件上传接口及单次切分/OCR 配置。

## 2026-07-24 RAG 向量投影与任务失败恢复

- 文档分块投影到检索表时必须优先读取真实 `embedding_vector`；旧版 `embedding_json` 只在维度与当前
  Embedding 配置完全一致时兼容使用，禁止把确定性短向量写入固定维度 pgvector。
- 文档流水线在嵌套事务保存点中执行，使单步数据库错误能先回滚再持久化任务失败状态，避免会话进入
  `PendingRollback` 后任务长期停留在排队中。
- 文档重处理幂等键必须控制在数据库字段长度以内；文档 ID、重试次数和请求 ID 已足以保证当前任务唯一性。

## 2026-07-24 FastAPI 整数枚举查询参数

- FastAPI 查询串中的枚举整数使用 `IntEnum`，不使用 `Literal[int]`；后者在当前 Pydantic 版本中会把 URL 字符串
  判为 `literal_error`，导致合法的 7/30/90 天或 10/20/50 分页值返回 422。
- 全局请求校验日志只记录路径、字段位置和错误类型，不记录输入值、请求体或凭据。

## 2026-07-24 多知识库检索、语义缓存与文档名称

- AI 搜索允许一次选择 1 至 10 个有权访问的知识库，前端使用 `workspaceIds`，OpenAPI 使用 `kb_ids`；缓存范围按排序后的
  知识库 ID 集合隔离。选择器和服务端都禁止同名知识库进入同一次查询，后端仍逐库执行权限校验。
- 答案缓存先查标准化问题的精确键，再使用本次检索复用的查询向量做语义相似匹配；默认阈值为 `0.95`，且问题类型、
  否定语义、数字和拉丁实体必须兼容。旧缓存或缺少向量的缓存只参与精确匹配，Redis 故障继续降级为正常 RAG。
- 同一知识库中的未删除文档名称按 `lower(trim(title))` 唯一。上传前通过名称冲突接口确认；冲突时必须由用户选择
  “替换已有文档”或“添加编号区分”，不得静默覆盖。回收站恢复遇到名称已被占用时自动添加编号并重新处理。
- 文档 Worker 对非内联任务在进入运行态及每个处理阶段提交一次状态，使上传、重处理进度可被轮询实时观察；
  前端仅在任务处于排队或运行中时按 1 秒轮询，终态或页面离开后停止轮询。导出任务采用同样的前端轮询规则。
- 无冲突预检后的上传使用 `new_version` 严格策略，并发同名返回 409 后重新确认；同一批文件自身重名时禁止
  `replace`。明确替换已有文档时必须递增并写回 `document.version`，保证缓存版本和内容哈希约束同步变化。
- 所有异步轮询必须绑定组件存活状态与请求代次；中止或过期请求不得更新新一轮状态，也不得在页面卸载后重新挂定时器。

## 2026-07-26 问答、导出与管理端预览

- 从首页或“我的空间”进入 AI 搜索页的提问属于一次性导航意图：真实 API 模式必须等待知识库和聊天模型选项加载完成后再提交，并在读取后清除路由状态，禁止使用演示模型 ID 抢跑请求或在刷新后重复提问。
- AI 搜索页使用单页对话布局，问题、流式答案和底部输入框处于同一页面；生成期间仅在用户仍靠近页面底部时自动跟随，用户主动向上阅读后不得强行抢回滚动位置。
- 问答导出任务在服务端私有目录保留规范化 Markdown 源内容；下载页切换 PDF、Word 或 Markdown 时由后端根据该源内容生成新的、可审计的导出记录，客户端不得回传历史答案正文。升级前仅 Markdown 任务可安全转换，缺少源内容的旧 PDF/Word 任务应提示重新导出。
- 管理端文档查看必须留在 `/admin/documents`，通过真实文档原件、Markdown 和分块接口在抽屉中预览；不得跳转到用户端知识库路由。
- 模型标识下拉仅提供按供应商与模型类型整理的常用建议，不作为白名单；管理员仍可输入供应商新发布或私有部署的自定义模型标识。

## 2026-07-26 会话式多知识库 RAG

- AI 搜索的会话能力统一扩展在 `/api/v1/retrieval/answer/stream`，继续保留多知识库、语义缓存、
  混合检索和重排；不得切回只支持单知识库的旧 `/chat/stream`。SSE 的 `start` 与 `done`
  必须返回真实 `conversation_id` 和消息 ID，缓存命中、无结果、正常生成均要保存用户与助手消息。
- `conversations.kb_id` 只作为旧接口兼容主知识库，`knowledge_base_ids` 保存 1 至 10 个知识库的完整
  固定范围。继续会话时后端必须校验所属用户、知识库权限和范围一致性；更换知识库范围必须新建对话。
- 首轮回答继续使用精确与高置信度语义缓存；同一对话中的完整独立问题也允许读取缓存，只有“那它呢”
  “继续说明”等依赖历史的省略式追问跳过缓存，避免把多轮能力与语义复用互相排斥。历史只注入最近的
  user/assistant 消息，并受字符预算约束；带历史生成的新答案仍不写入全局答案缓存。
- “我的空间”只向搜索页传递一次性知识库预选，不填固定问题、不自动提交；搜索输入在提交后立即清空。
  对话在每轮完成时已实时持久化，“新建对话”只清空当前页面并保留知识库和模型选择。
- 用户主导航使用真实“对话历史”替换“收藏内容”；`/conversations` 是规范地址，旧 `/favorites`
  只做兼容重定向。历史问题正文不得写入 URL，使用不含敏感内容的会话 ID 恢复页面。
- 当前 Alembic `alembic_version.version_num` 长度为 32，迁移 revision 标识必须控制在 32 个字符以内；
  迁移必须在 Docker PostgreSQL 环境实际执行，不能只依赖静态检查。

## 2026-07-26 单页搜索与默认检索范围

- 用户工作区根路径 `/` 是 AI 搜索唯一主页面，问题、历史回答、流式新回答和底部输入框必须处于同一页面；
  `/search` 只保留查询参数兼容重定向。用户搜索流程不再展示高频任务、知识共建、独立查询摘要或辅助信息侧栏。
- 没有空间预选或历史会话范围时，前端必须优先选择第一份 `ready_document_count > 0` 的可访问知识库；
  所有范围均无已处理文档时才回退首项，并禁止对完全空的范围发起检索，避免把“无引用”误判为 RAG 故障。
- “我的下载”中的问答任务点击下载后必须先显示 Markdown、Word 和 PDF 三格式选择，再打开对应格式的系统保存位置；
  表格格式列只描述当前记录，不再承担容易被忽略的下载格式切换。
- 本地 Docker Compose 命令必须显式使用 `--env-file deploy/env/.env`；直接依赖 Compose 默认 `.env`
  可能让迁移容器与现有 PostgreSQL 数据卷使用不同口令。

## 2026-07-26 流式会话、评测分数与原生交付

- FastAPI `StreamingResponse` 的生成器不能继续使用请求依赖创建的数据库会话；流式路由只捕获用户 ID，
  生成器内部重新创建会话并读取用户，使检索、会话保存、指标和审计共享完整 SSE 生命周期。
- 混合检索的 RRF 值只用于内部排序；阈值判断和对外 `SearchHit.score` 必须统一使用
  rerank、向量或关键词的最终相关性分数，避免页面显示低分却判定命中。
- 检索测试集及运行记录按当前用户可访问的知识库过滤；管理看板的“评测命中率”来自真实测试运行，
  不能把是否存在引用或检索命中描述为生成答案正确率。
- 集成测试必须显式使用 `TEST_DATABASE_URL` 和独立临时 schema，不得回退到运行环境 `DATABASE_URL`；
  创建 schema 前必须导入全部 ORM 模型，防止测试污染演示数据库或因外键表缺失失败。
- Linux 镜像中的知识库服务、权限过滤、检索服务和许可证校验可使用 Cython 编译为 `.so`，运行镜像删除
  对应源码。Ed25519 公钥只在构建期注入扩展，私钥不进入仓库；Python 适配层只获得许可证布尔状态，
  不返回序列号、许可证正文或签名。
- 知识库编辑者应具备本部门企业文档的上传、重处理、可恢复删除和恢复闭环；普通用户仍只能查看企业
  文档，并在自己的个人知识库中上传文档。

## 2026-07-27 上传进度与搜索操作收敛

- 异步文档任务完成时，进度组件必须把最终任务状态同步给父页面；父页面先保存终态再刷新文档列表。
  加载态导致组件重挂载时，已经终止的任务不得重新轮询或再次发出完成事件，避免请求循环和 `429`。
- AI 搜索页不再提供答案、检索结果或预览文档的收藏入口；用户需要保留的问答统一依靠真实对话历史，
  搜索页只保留复制、下载、反馈、引用预览和新建对话操作。

## 2026-07-27 知识库归档生命周期

- 知识库归档表示“保留数据但离线只读”：知识库、原文、Markdown、分块、向量和历史记录继续保留，
  管理端仍可查看并恢复，但禁止上传、重新处理、文档删除/恢复和 RAG 检索。
- 用户工作区必须通过 `/api/v1/knowledge-bases/available` 获取可用知识库；管理端列表可以继续展示归档项。
  后端知识库权限范围必须在数据库查询层排除非 `active` 状态，禁止只依赖前端隐藏。
- RAG 请求显式携带归档或无权限知识库时必须失败并提示重新选择，不得静默缩小范围或改查其他知识库；
  恢复为 `active` 后，原有文档和向量重新进入用户列表与检索范围。

## 2026-07-27 RAG 未命中与引用一致性

- “检索到相似片段”不等于“片段足以支持回答”。流式 RAG 必须在模型完成答案后再发布引用；模型明确
  表示文档资料不足时，响应、会话消息、缓存和页面中的引用均为空，不显示“结论依据”或原始结果。
- 无文档依据的答案不写入语义答案缓存；前端仍需兼容历史会话和旧服务端的矛盾数据，根据最终回答
  防御性隐藏旧引用。正常命中回答继续保留可核验的编号引用。
