...
API 启动后会在专用线程池后台加载 `BanTopics` 与 `PromptInjection`。首次运行会把
约 1.1 GB 模型文件写入 `kb-llm-guard-model-cache`，后续重启复用该卷。预热期间
`/api/v1/health/live` 保持 200，但 `/api/v1/health/ready` 的 `llm_guard` 为
`warming` 并返回 503；出现以下日志后才可接收检索和问答流量：

```text
llm_guard_models_ready
```

可用以下命令查看状态，不要删除模型缓存卷作为常规重启手段：

```bash
docker compose --env-file deploy/env/.env -f deploy/docker-compose.yml logs --tail=100 api-server
docker compose --env-file deploy/env/.env -f deploy/docker-compose.yml ps
```
...
| `/api/v1/health/ready` | 数据库、Redis、存储和 LLM Guard 均就绪时 200，否则 503 |
...
7. `llm_guard` 长时间为 `warming`：查看 API 日志和模型缓存卷容量，确认容器可访问模型源且至少有 4 GiB 可用内存。
