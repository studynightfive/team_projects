...
> 状态说明：其中答案导出功能继续有效；LLM Guard 模型扫描已于同日因正常医疗问题误判
> 而撤销，详见 [撤除方案](../design/remove-llm-guard-backend/PRD.md)。以下守卫内容仅作为
> 历史实施记录，不再代表当前运行状态。
...
AI 搜索答案区使用“下载答案”入口，下载弹窗以分段控件直接展示 Markdown、Word、PDF
三种格式，默认选择 Markdown；确认按钮和文件名随当前格式同步变化。真实 API 请求分别传递
`markdown`、`docx`、`pdf`，生成记录继续保存在“我的下载”。
