...
检索指标的业务归属以知识库为事实来源：有引用时使用首条最终引用的 `kb_id`，无引用时
回退到请求中首个知识库；指标写入前读取该知识库的 `department_id`，不再复制提问用户的
部门。看板按知识库所属部门过滤，仅对无法恢复知识库的历史指标回退到指标快照部门。
数据迁移先通过主商品文档回填，再用指标完成前紧邻的助手消息所属会话回填旧流式问答，
最后统一刷新部门快照。无法可靠判断的旧数据保持未归属，禁止猜测到任意部门。

管理端顶部原“演示 / 生产”选择器只有前端局部状态，不控制 API、模型或部署环境，删除该
无效入口；运行环境继续只由部署配置决定。
