...
- 问答任务的“下载”按钮打开格式选择弹窗，使用分段控件展示 Markdown、Word 和 PDF。确认后才调用原文件下载或 `POST /api/v1/exports/{export_id}/convert`，随后打开对应扩展名的系统保存选择器。
- 表格“格式”列只显示当前记录格式，不再承担隐蔽的下载格式选择，避免用户误以为只能下载 Markdown。

## 验证

- 后端：任务持久化、三种主格式、失败记录、再次下载权限和路径检查。
- 前端：搜索页和下载页弹窗格式切换、取消不请求、成功保存、列表问答名称与再次下载。
