...
- `WorkspaceShell` 只协调共享网格、侧栏、顶栏槽位、移动抽屉、底部导航和本地个人资料弹窗。
...
- 个人资料弹窗开关属于 `WorkspaceShell` 局部状态；身份数据继续来自既有固定展示数据，不新增 Store 或接口。
...
- 个人资料弹窗复用顶栏头像与账号菜单入口，展示身份、工作区、认证接入状态和资料来源；用户端唯一主要操作进入 `/preferences`。
...
- 个人资料弹窗桌面使用双列概览，767px 以下降为单列；弹窗内容和操作必须保持在视口内。
...
浏览器脚本覆盖 23 个 M01 用例：用户、管理、登录的 1920/1440/1280/375，用户/管理折叠 1440，个人资料弹窗的 1440/1280/375，以及 403/404 的 1440/1280/375。检查顶栏、侧栏、个人资料弹窗、四指标、Sparkline、审计表头、登录错误/显隐、移动抽屉、触控尺寸、根级溢出、控制台错误和业务 API 请求。
